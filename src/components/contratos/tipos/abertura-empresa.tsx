...
const useStyles = () => {
  ...
  return useMemo(() => {
    return StyleSheet.create({
      ...
      paymentContainer: {
        flexDirection: "column", // Coloca os quadros um abaixo do outro
      ...
      payment: {
        flexDirection: "row", // Mantém o conteúdo dos quadros em linha
        margin: 0, // Remove margem entre os quadros
        padding: 0, // Remove padding entre os quadros
      },
      paymentDetails: {
        borderColor: "#000",
...
export const AberturaDeEmpresa = ({ contract }: any) => {
        ...
        <View style={styles.paymentContainer} wrap={false}>
          {services.map((service: any, key: any) => {
            if (service.type === "0") {
              ...
              return (
                <View style={styles.payment} key={key}>
                  <View style={styles.paymentDetails}>
                    <Text
                      style={styles.body2}
                    >{`Objeto ${objectLabels[key]}`}</Text>
              ...
              return (
                <View style={styles.payment} key={key}>
                  <View style={styles.paymentDetails}>
                    ...
                    <Text style={styles.body2}>
                      <Text style={styles.boldText}>Faixa de faturamento</Text>
                      Até - {formatBRL(service.billingRange)}
                    </Text>
                  </View>
                  <View style={styles.paymentDetails2}>
                    {service.paymentEntry ? (
      ...
      <Page size="A4" style={styles.page} wrap={true}>
        ...
        <View style={styles.contract}>
          <View>
            ...
            {contract.persons?.map((person: any, key: any) => (
              <Text style={[styles.body2, styles.contractText]} key={key}>
      ...
      <Page size="A4" style={styles.page} wrap={true}>
        ...
        <View style={styles.contract}>
          <View>
            ...
            {contract.persons?.map((person: any, key: any, array: any) => (
              <Text style={[styles.body2, styles.contractText]} key={key}>
                <Text style={styles.boldText}>{person.name}</Text>,{" "}
                {person.nationality},{person.maritalStatus} inscrito no CPF sob
